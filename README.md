...
A distributed task queue system with **priority-based processing**, **delayed task execution**, **async/sync workers**, and a **REST API** for managing tasks at scale.

## ✨ Key Features

- **Priority-based Task Queue** - Higher priority tasks are processed first
- **Delayed Task Execution** - Schedule tasks to execute at specific times
- **Dual Worker Models** - Choose sync (threading) or async (asyncio) processing
- **Redis-backed Broker** - Reliable, distributed task storage and management   
- **Dead-Letter Queue** - Failed tasks are automatically routed to DLQ
- **Retry Logic** - Exponential backoff retry mechanism for fault tolerance
- **REST API** - HTTP endpoints for complete task management
- **Task Status Tracking** - Real-time monitoring of task execution
- **Performance Statistics** - Built-in metrics and analytics
...
```python
from src import TaskProducer, TaskWorker, TaskExecutor

# Create producer and submit tasks
producer = TaskProducer()
task_id = producer.submit_task('send_email', {
    'to': 'user@example.com',
    'subject': 'Hello'
})

# Create worker and define task handlers
executor = TaskExecutor()
executor.register('send_email', handle_send_email)

worker = TaskWorker(executor=executor)
worker.run()
```
...
| Document | Purpose |
|----------|---------|
| [Architecture Overview](docs/IMPLEMENTATION_SUMMARY.md) | System design and component interactions |
| [API Documentation](docs/API_DOCUMENTATION.md) | REST API reference and examples |
| [Async Worker Guide](docs/ASYNC_WORKER_README.md) | Using AsyncTaskWorker for I/O-bound tasks |
| [Priority Queue Guide](docs/PRIORITY_QUEUE_README.md) | Priority-based task processing |
| [Delayed Tasks Guide](docs/DELAYED_TASKS_README.md) | Scheduling tasks for future execution |
| [Quick Start - Delayed](docs/QUICK_START_DELAYED.md) | Delayed task execution examples |
...
│   ├── test_delayed_tasks.py
│   └── test_delayed_realistic.py
│
├── examples/            # Example scripts
│   ├── demo_delayed_priority.py
│   ├── api_quickstart.py
│   └── SYNC_VS_ASYNC_COMPARISON.py
│
...
└── docs/                # Documentation
    ...
    ├── ASYNC_WORKER_README.md
    ├── PRIORITY_QUEUE_README.md
    └── DELAYED_TASKS_README.md
```
...
### Producer
Submits tasks to the queue with optional priority and delay settings.

### Broker (Redis)
Manages task storage, retrieval, and status tracking using Redis sorted sets for priority ordering.

### Worker
Processes tasks from the queue. Choose between:
- **TaskWorker**: Synchronous, thread-based for CPU-bound tasks
- **AsyncTaskWorker**: Asynchronous, event-loop based for I/O-bound tasks

## 🔧 Core Concepts

### Tasks
Tasks have:
- **Type**: Classification for handler routing
- **Payload**: Data to process
- **Priority**: 0-100 (higher = processed first)
- **Status**: PENDING → PROCESSING → COMPLETED/FAILED
- **Retry Count**: Automatic retries with exponential backoff
...
### Queue Management
- Normal queue for immediate tasks
- Delayed queue for future execution
- Dead-letter queue for failed tasks

## 📊 Comparison: Sync vs Async Worker

| Feature | Sync Worker | Async Worker |
|---------|------------|-------------|
| Execution Model | Threading | Asyncio |
| Best For | CPU-intensive | I/O-intensive |
| Max Concurrency | 100-200 | 1000+ |
| Memory per Task | ~2MB | ~50KB |
| Task Context | `data` | coroutines |

See [SYNC_VS_ASYNC_COMPARISON.py](examples/SYNC_VS_ASYNC_COMPARISON.py) for detailed comparison.
...
# Delayed tasks tests
python -m pytest tests/test_delayed_tasks.py

# Realistic integration tests
python -m pytest tests/test_delayed_realistic.py
```
...
curl -X POST http://localhost:8000/tasks \
  ...
  -d '{"type": "send_email", "priority": 5, "data": {...}}'
...
- Check if worker is running: `worker.run()`
- Verify Redis connection: `broker.health_check()`
- Check task type is registered: `executor.has_handler('type')`

### High memory usage
- Consider using async worker for I/O tasks
- Reduce task payload size
- Monitor dead-letter queue for failures

### Slow processing
- Increase worker threads: `worker.run_parallel(8)`
- Use async worker for I/O-bound tasks
- Profile handler execution time

## 📝 Examples

See [examples/](examples/) directory for:
- `api_quickstart.py` - REST API usage examples
- `demo_delayed_priority.py` - Priority and delay demonstration
- `SYNC_VS_ASYNC_COMPARISON.py` - Worker comparison benchmark
...
**Built with ❤️ for reliable, scalable task processing**
