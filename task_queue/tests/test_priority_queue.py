...
def test_priority_queue():
    ...
    print_section("Priority Queue Test - ZADD and ZPOPMIN")
    ...
    for task_type, data, priority in tasks_to_submit:
        ...
        print(f"  [OK] {task_type}: ID={task_id[:12]}... (priority={priority})")
    
    # Test 2: Check queue size
    print("\n[2] Checking queue state...")
    status = client.get_queue_status()
    print(f"  [OK] Queue size: {status['queue_size']} tasks")
    print(f"  [OK] Processing: {status['processing_count']}")
    ...
    for task_type, priority in dequeue_order:
        if priority > previous_priority:
            print(f"  [FAIL] Wrong order: {task_type} (priority={priority}) after priority={previous_priority}")
            all_correct = False
        else:
            print(f"  [OK] {task_type}: priority={priority} (correctly ordered)")
            previous_priority = priority
    
    if all_correct:
        print("\n  [SUCCESS] Priority queue working correctly! Tasks dequeued in correct priority order.")
    else:
        print("\n  [FAILED] Priority ordering failed!")
    ...
    print(f"  [OK] Queue (sorted set) cardinality: {queue_size}")
    print(f"  [OK] Processing (sorted set) cardinality: {processing_size}")
    ...
    # Test 6: Demonstrate ZADD and ZPOPMIN usage
    print("\n[6] Redis operations used:")
    print("  • ZADD: Adds tasks to sorted set with score = -priority * 1000 + timestamp")
    print("  • ZPOPMIN: Pops task with minimum score (highest priority first)")
